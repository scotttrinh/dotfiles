...
let
  ...
  envHelperText = ''
    ...
    # uv derives these names from the `socket-firewall` index name in uv.toml.
    # Keep them aligned with the helper generated by Socket Firewall 1.8.2.
    export UV_INDEX_SOCKET_FIREWALL_USERNAME="${cfg.username}"
    if [ -n "''${SOCKET_PASSWORD_B64:-}" ]; then
      export UV_INDEX_SOCKET_FIREWALL_PASSWORD="$(printf '%s' "''${SOCKET_PASSWORD_B64}" | base64 -d 2>/dev/null)"
    fi
...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    # The MDM reinstall script replaces these Home Manager symlinks with regular
    # files. Reassert ownership instead of accumulating one-shot backup files,
    # which makes repeated reinstall/activation cycles fail on backup collisions.
    # programs.zsh uses "./.zshenv" as its generated home.file key.
    home.file."./.zshenv".force = true;
    home.file.".config/socket-firewall/env.sh" = {
      force = true;
      text = envHelperText;
    };
